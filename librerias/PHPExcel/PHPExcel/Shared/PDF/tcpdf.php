				...
				foreach ($this->PageAnnots[$n] as $key => $pl) {
					...
					switch (strtolower($pl['opt']['subtype'])) {
						...
						case 'link': {
							if(is_string($pl['txt'])) {
								// external URI link
								$annots .= ' /A <</S /URI /URI '.$this->_datastring($this->unhtmlentitiess($pl['txt']), $annot_obj_id).'>>';
							} else {
								// internal link
								$l = $this->links[$pl['txt']];
	...
	protected function getHtmlDomArray($html) {
		...
		if (preg_match_all('/<cssarray>([^\<]*)<\/cssarray>/isU', $html, $matches) > 0) {
			if (isset($matches[1][0])) {
				$css = array_merge($css, unserialize($this->unhtmlentitiess($matches[1][0])));
			}
		...
		$csstagarray = '<cssarray>'.htmlentitiess(serialize($css)).'</cssarray>';
		...
		while ($elkey < $maxel) {
			...
			if (preg_match($tagpattern, $element)) {
				...
				$dom[$key]['value'] = stripslashes($this->unhtmlentitiess($element));
				$dom[$key]['parent'] = end($level);
			}
			++$elkey;
			++$key;
...
Putting 1 is equivalent to putting 0 and calling Ln() just after. Default value: 0.
	...
	public function hyphenateText($text, $patterns, $dictionary=array(), $leftmin=1, $rightmin=2, $charmin=1, $charmax=8) {
		$text = $this->unhtmlentitiess($text);
